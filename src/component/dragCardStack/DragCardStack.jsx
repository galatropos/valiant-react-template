...
function getPhaseZ(offset, zPhase) {
  if (zPhase === 'base') {
...
export default function DragCardStack({
  cards = DEFAULT_CARDS,
  autoPlay = true,
  autoPlayDelay = 2200,
  stepSize = 140,
  ...
  const [zPhase, setZPhase] = useState('base');
  ...
  const states = useMemo(() => {
    ...
    const sideY = heightValue * middleYRatio;
  ...
  const visibleStates = useMemo(() => {
    return cards.map((_, index) => {
      ...
      return {
        ...interpolated,
        z: getPhaseZ(offset, zPhase),
      };
    });
  }, [cards, previewActive, previewProgress, states, total, zPhase]);

  useEffect(() => {
    zPhaseRef.current = 'base';
    setPreviewValue({
  ...
  useEffect(() => {
    function flushPreviewFrame() {
      ...
      const { nextPreviewActive, nextPreviewProgress, nextPhase } =
        pendingPreviewRef.current;

      if (zPhaseRef.current !== nextPhase) {
        zPhaseRef.current = nextPhase;
        setZPhase(nextPhase);
      }
